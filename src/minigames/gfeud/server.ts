...
var sendPrompt = function (){
  SocketManager.broadcast({GFeudPrompt: formatAnswers().join("<br/>")});
}
...
var httpRequestCallback = function(html: string){
  ...
  var p_words = prompt.split(" ");
  for(var j = 0; j <p_words.length;j++){
    words.delete(p_words[j]);
  }
...
game.endingCleanup = function() {
  SocketManager.broadcast({"GFeudPrompt": answers.join("<br/>")});
}
...
game.listener = function(object, socket){
  if (object.ProposeQuizzModuleAnswer){
    var proposal = object.ProposeQuizzModuleAnswer.toLowerCase().split(" ");
    var points = 0;
    ...
    if (points > 0) {
        game.scores.incrementScore(socket.player_data().player_id, points);
        sendPrompt();
    }
  }
}

game.timeAfterEnd = 15000;
game.startWithDeadline(90000);
